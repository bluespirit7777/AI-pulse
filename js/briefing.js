...
export function renderBrief(root, summary, signals = [], now = Date.now()) {
  ...
  root.querySelectorAll('[data-brief-preview]').forEach(button=>button.addEventListener('click',()=>{
    ...
    openDialog('Story preview',`<p class="eyebrow">FROM THE ORIGINAL SOURCE</p><h3>${esc(story.title)}</h3><p class="meta" translate="no">${esc(story.sourceName||'')} · ${esc(story.dateISO?.slice(0,10)||'')}</p><p class="story-full-excerpt">${esc(story.desc||'')}</p><p class="meta">This is the publisher’s excerpt. Open the source for the complete story.</p>${sourceLink(story.url,'Read the full story')}`);
  }));

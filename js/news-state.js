...
export function filterNews(signals,state,entities=[],now=Date.now()) {
 ...
 return signals.filter(s=>{
  ...
  const text=[s.title,s.desc,translateText(s.title,'th'),translateText(s.desc,'th'),s.sourceName,s.category,translateText(s.category,'th'),entitiesText].join(' ').toLowerCase();
